...
Spring 트랜잭션은 `@Transactional`을 기준으로 프록시가 메서드 호출을 감싸고, 트랜잭션 시작, commit, rollback을 대신 처리하는 기능이다.
...
| `readOnly = true` | 읽기 전용 힌트 | 쓰기 방지 보장이 아니라 최적화 힌트 성격으로 이해한다 |
...
Spring의 `@Transactional`은 프록시를 통해 메서드 호출을 감싸는 선언적 트랜잭션 기능이다.
