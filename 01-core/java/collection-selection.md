...
Java 컬렉션은 데이터를 어떻게 다시 사용할지에 따라 선택합니다.
순서가 중요하고 중복을 허용하는 목록이면 `List`, 중복 제거가 목적이면 `Set`, id나 code로 빠르게 찾아야 하면 `Map`을 우선 고려합니다.

실무에서는 DB에서 조회한 목록을 응답으로 조립할 때 `List`를 매번 순회하면 성능이 나빠질 수 있어서, 반복 조회가 있으면 `Map`으로 변환해 key 기반 조회를 사용합니다.
반대로 데이터가 작고 한 번만 순회한다면 `Map`으로 바꾸는 것이 오히려 복잡할 수 있습니다.

또 `HashSet`과 `HashMap`은 순서를 보장하지 않고, 객체를 key나 Set 요소로 쓸 때는 `equals/hashCode` 기준이 중요합니다.
그래서 컬렉션 선택은 성능뿐 아니라 순서, 중복, 동등성, 동시성 요구사항을 함께 보고 결정해야 합니다.

## 꼬리 질문

> [!question]- `List`, `Set`, `Map`은 각각 어떤 상황에서 선택하는가?
> 순서가 있고 중복을 허용하면 `List`, 중복 제거가 목적이면 `Set`, key로 빠르게 찾아야 하면 `Map`을 선택합니다.

> [!question]- `List.contains()`를 반복해서 쓰면 어떤 문제가 생길 수 있는가?
> `List.contains()`는 O(n)이라 반복문 안에서 호출하면 O(n²)이 됩니다. `Set`으로 변환하면 O(n)으로 줄어듭니다.

> [!question]- `HashSet`은 중복을 어떤 기준으로 판단하는가?
> `hashCode()`로 bucket을 찾고 `equals()`로 같은 객체인지 확인합니다. 둘 다 재정의하지 않으면 참조 비교가 됩니다.

> [!question]- `Collectors.toMap()`에서 중복 key가 생기면 어떻게 처리할 수 있는가?
> 기본적으로 `IllegalStateException`이 발생합니다. 세 번째 인자로 merge 전략을 지정해서 `(old, new) -> new`처럼 처리합니다.

> [!question]- 응답 순서가 중요할 때 어떤 컬렉션을 고려해야 하는가?
> `ArrayList`, `LinkedHashSet`, `LinkedHashMap`은 입력 순서를 유지합니다. `HashMap`과 `HashSet`은 순서를 보장하지 않습니다.
